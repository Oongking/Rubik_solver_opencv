    ...
    if Num_P==9 and "(E)" not in S_pattern:
        ...
        if S_pattern == Face[face_num-1]:
            ...
            if face_num == 1 or face_num == 2 or face_num == 3:
                percentage = SequenceMatcher(None, S_pattern, Face[face_num-1]).ratio()
                if percentage < 0.8:
                    Toggle = 0
            elif face_num == 4:
                percentage = SequenceMatcher(None, S_pattern, Face[face_num-1]).ratio()
                percentage_1 = SequenceMatcher(None, S_pattern, Face[0]).ratio()
                # print('percentage_1 : ',percentage_1)
                if percentage < 0.8 and percentage_1 < 0.8:
                    Toggle = 0
                    Face_step = 0
                if percentage_1 > 0.8:
                    Face_step = 1
        ...
        if Num_P==9 and "(E)" not in S_pattern and Toggle == 0:
